...
class SymbolResponseDtoValidatorTest {

  @Mock
  private ConstraintValidatorContext context;

  @InjectMocks
  private SymbolResponseDtoValidator validator;
  ...
  void testSymbolResponseDtoValidatorWhenCurrencyIsDiffIsValid(Currency currency) {
    //given
    final var symbol = Mockito.mock(SymbolResponseDto.class);
    //when
    Mockito.when(symbol.currencyBase()).thenReturn(currency);
    Mockito.when(symbol.currencyQuote()).thenReturn(Arrays.stream(Currency.values()).filter(c -> !currency.equals(c)).findFirst().orElse(null));
    //then
  ...
  @ParameterizedTest
  @EnumSource(Currency.class)
  void testSymbolResponseDtoValidatorWhenCurrencyIsEqualIsInvalid(Currency currency) {
    //given
    final var symbol = Mockito.mock(SymbolResponseDto.class);
    //when
    Mockito.when(symbol.currencyBase()).thenReturn(currency);
